...
**prompt:**
"Build a RAG demo over PDFs using ChromaDB, sentence-transformers, and pypdf, reusing the OpenRouter client from ChatApp.ipynb. [full spec of requirements]"

**result:**
Claude built the full pipeline in one pass — PDF discovery, chunking, embedding, ChromaDB storage, retrieval, and the reused OpenRouter call — and tested it end-to-end before I reviewed it.
...
**result:**
Condensed all markdown explanations and docstrings while keeping the same logic.
...
**result:**
got a report grounded in my actual pipeline — retrieval inspection, similarity scores, pydantic validation, and the task 4 tests. read through it and checked the scenarios against what i'd actually seen while testing before keeping them in.
...
**result:**
got suggestions like adding mypy cache, logs/coverage, packaging folders, and environment file variants. also discussed whether generated outputs should stay tracked so the saved JSON answers are visible for review. turned out my current setup was already fine, so i only added the extra ignore entries that made sense.
